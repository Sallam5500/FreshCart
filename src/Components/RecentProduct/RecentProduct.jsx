  ...
  return<>
{
    loading ? (
      ...
      <div className="mx-auto flex flex-wrap gap-y-4 py-8 justify-center  ">
        {product.map((product) => {
          const isFavourite = wishList?.data?.some((item) => item.id === product.id);
  
          return (
              <div className="product p-2 rounded-lg group main-w-[200px] w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/6 px-1  ">
                  ...
                  <div className='flex justify-between py-3'>
                      <button onClick={()=> addProductToCart(product.id)} className='btn w-full'>AddToCart</button>
                      {isFavourite ? (
                <button
